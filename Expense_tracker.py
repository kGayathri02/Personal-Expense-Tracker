...
if uploaded_file is not None:
    try:
        ...
        def data_clean(df):
            ...
            df = df.drop(columns=['Subcategory', 'Note.1', 'Account.1'], errors='ignore')
            df.fillna({'Note': 'Random'}, inplace=True)
            df.loc[df['Amount'] != df['INR'], 'Amount'] = 1120.72
        ...
        try:
            ...
            st.error(f"An error occurred while cleaning the data: {e}")
        ...
        tab1,tab2,tab3,tab4,tab5,tab6,tab7= st.tabs(['Home', 'Income & Expense', 'Category-wise Breakdown', 'Highest/Lowest Spending', 'Income-Expense Ratio','Cash-flow Over Time', 'Conclusion'])    
        with tab1:
